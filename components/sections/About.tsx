...
const iconMap: Record<string, any> = {
  Server, Shield, Layers, Activity
};

export default function About({ data }: AboutProps) {
  const titleToUse = data?.title || "The Engineer Behind the Firewall";
  const subtitleToUse = data?.subtitle || "Architecting secure perimeters, mitigating vulnerability points, and ensuring network resiliency.";
  const biosToUse = data?.bioParagraphs || [
  ...
  const locationToUse = data?.locationNode || "Dallas, TX Node";
  const statusToUse = data?.status || "STATUS: OPERATIONAL_OK";
    ...
    <section id="about" className="py-24 bg-background relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
          ...
          >
            {biosToUse.map((para, idx) => (
              <p
                key={idx}
                className={
                  idx === 0
                    ? "font-sans text-lg md:text-xl text-text-primary leading-relaxed font-bold"
                    : "font-sans text-sm sm:text-base text-text-muted leading-relaxed"
                }
              >
                {para}
              </p>
          ...
          >
            {statsToUse.map((stat, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="p-6 panel-glass border border-border-color rounded-lg relative flex flex-col justify-between hover:border-accent/40 hover:shadow-[0_0_20px_rgba(0,102,255,0.08)] transition-all duration-300 group cursor-default"
                >
                  {/* Micro tech label */}
